...
#pragma ident	"@(#)bootops.c	1.9	05/10/17 SMI"
...
bop_putsarg(struct bootops *bop, const char *string, ...)
{
	...
	while (*fmt) {
		...
		case 's':
			/* use uintptr_t to suppress the gcc warning */
			arg = (uint_t)(uintptr_t)va_arg(ap, char *);
			break;
		case 'p':
			arg = (uint_t)(uintptr_t)va_arg(ap, void *);
			break;
		case 'd':
		case 'D':
		case 'x':
		...
		case 'O':
			if (ells == 0)
				arg = (uint_t)va_arg(ap, uint_t);
			else if (ells == 1)
				arg = (uint_t)va_arg(ap, ulong_t);
			else
				arg = (uint_t)va_arg(ap, uint_t);
			break;
		default:
