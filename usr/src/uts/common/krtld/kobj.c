...
#pragma ident	"@(#)kobj.c	1.168	06/06/13 SMI"
...
#define	_moddebug	moddebug/* get_weakish_int(&moddebug) */
...
#if !defined(__sparc) && !defined(__powerpc)
	{
...
attr_val(val_t *bootaux)
{
	...
	int first_phdr = 1; /* for powerpc temp fix */
	for (i = 0; i < phnum; i++) {
		phdr = (Phdr *)(bootaux[BA_PHDR].ba_val + i * phsize);

		if (phdr->p_type != PT_LOAD)
			continue;
#if defined(__powerpc) /* XXX The temp way to handle GNU ld-generated unix binary */
		if (first_phdr) {
...
load_linker(val_t *bootaux)
{
	...
	cp->mod_mp = mp;
	/* mp->hdr = *(Ehdr *)bootaux[BA_LDELF].ba_ptr; //gcc compile this to memcpy */
	bcopy(bootaux[BA_LDELF].ba_ptr, &mp->hdr, sizeof(mp->hdr));
...
/*	mp->text_size = _etext - mp->text; */
	mp->text_size = mp->text - _etext; /* powerpc */
