 ...
 *
 * Portions Copyright 2005 Cyril Plisko <cyril.plisko@mountall.com>
 */
...
/*
 * Next set of definitions characterize Solaris on PowerPC platform.
 * While PowerPC architecture defines both big endian and little endian
 * modes and despite the fact that the first port of Solaris to PowerPC
 * architecture (done by SunSoft) was little endian we are choosing
 * big endian.  Suffice it to say that there are implementations that
 * do not implement little endian mode at all.
 */
...
#define	__powerpc
#if !defined(__ppc)
#define	__ppc	/* convenient shortcut */
#endif
#endif
...
#define	__powerpc64
#if !defined(__ppc64)
#define	__ppc64	/* convenient shortcut */
#endif
#endif
 ...
 * This is governed by the LE bit in the Machine State Register (MSR).
 * So, it is up to us to choose.  Solaris/PPC 2.6 chose little-endian,
 * for reasons that made sense at the time.  Current thinking is that
 * we would prefer big-endian, for the following reasons, among others:
 *   + PowerPC processors power up in big-endian mode;
 *   + PTEs are stored big-endian no matter what mode the processor
 *     is in, so there is a bit less work to access and modify PTEs.
 * Solaris/PPC ought to work both ways.  For now, we default to big-endian.
 * But that is only if it has not already been set by the compiler or Makefiles.
 * The GNU C compiler for PowerPC defines _BIG_ENDIAN or _LITTLE_ENDIAN,
 * built-in.
 */
