...
#pragma ident	"@(#)dmfe_ndd.c	1.7	05/06/08 SMI"
...
dmfe_nd_ioctl(dmfe_t *dmfep, queue_t *wq, mblk_t *mp, int cmd)
{
	...
	case DMFE_ND_SET:
		...
		if (ndp->ndp_info == info)
			return (IOC_REPLY);
		if (ndp->ndp_val) {
			dmfep->link_down_msg = " (autonegotiation enabled)";
			dmfep->link_up_msg = " (autonegotiated)";
		} else {
			dmfep->link_down_msg = " (autonegotiation disabled)";
			dmfep->link_up_msg = " (forced)";
		}
		return (IOC_RESTART);
	}
}
