...
#pragma ident	"@(#)server_acl.c	1.7	06/04/29 SMI"
...
static const aop_t acl_op_table[] = {
    ...
    { 'i',	ACL_INQUIRE },
    { 'l',	ACL_LIST },
    { 'p',	ACL_IPROP },		/* SUNW IProp */
    { 's',	ACL_SETKEY },
    { 'u',	ACL_MIGRATE },		/* pam_krb5_migrate */
    { 'x',	ACL_ALL_MASK },
    { '*',	ACL_ALL_MASK },
    { '\0',	0 }
};
...
#define ACL_SYN_ERR_MSG dgettext(TEXT_DOMAIN, \
			"%s: syntax error at line %d <%10s...>\n")
#define ACL_CANTOPEN_MSG dgettext(TEXT_DOMAIN, \
...
__kadm5_get_priv(void *server_handle, long *privs, gss_name_t client)
{
	...
	aent_t		*aentry;
	gss_buffer_desc	caller_buff;
	gss_OID		caller_oid;
	krb5_principal	caller_principal;
