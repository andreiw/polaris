...
#pragma ident	"@(#)dhcp_network.c	1.5	05/06/08 SMI"
...
dsvcnis_dnrec_to_entryobj(const dn_rec_t *dnp, nis_object **eop)
{
	...
	/* flags */
	ENTRY_VAL(nop, F_AUTO_DN) = (dnp->dn_flags & DN_FAUTOMATIC) ?
	    strdup("1") : strdup("0");
	ENTRY_LEN(nop, F_AUTO_DN) = 2;

	ENTRY_VAL(nop, F_MANUAL_DN) = (dnp->dn_flags & DN_FMANUAL) ?
	    strdup("1") : strdup("0");
	ENTRY_LEN(nop, F_MANUAL_DN) = 2;

	ENTRY_VAL(nop, F_UNUSABLE_DN) = (dnp->dn_flags & DN_FUNUSABLE) ?
	    strdup("1") : strdup("0");
	ENTRY_LEN(nop, F_UNUSABLE_DN) = 2;

	ENTRY_VAL(nop, F_BOOTP_DN) = (dnp->dn_flags & DN_FBOOTP_ONLY) ?
	    strdup("1") : strdup("0");
	ENTRY_LEN(nop, F_BOOTP_DN) = 2;

	if (ENTRY_VAL(nop, F_AUTO_DN) == NULL ||
	    ENTRY_VAL(nop, F_MANUAL_DN) == NULL ||
	    ENTRY_VAL(nop, F_UNUSABLE_DN) == NULL ||
	    ENTRY_VAL(nop, F_BOOTP_DN) == NULL) {
		nis_destroy_object(nop);
		return (DSVC_NO_MEMORY);
	}
	...
	(void) snprintf(ENTRY_VAL(nop, LEASE_DN), LEASE_BUF_DN, "%u",
	    dnp->dn_lease);
...
dsvcnis_entryobj_to_dnrec(const nis_object *op, dn_rec_t **dnpp)
{
	...
	/* flags */
	dnp->dn_flags = 0;
	if ((ENTRY_VAL(op, F_AUTO_DN))[0] == '1')
		dnp->dn_flags |= DN_FAUTOMATIC;
	if ((ENTRY_VAL(op, F_MANUAL_DN))[0] == '1')
		dnp->dn_flags |= DN_FMANUAL;
	if ((ENTRY_VAL(op, F_UNUSABLE_DN))[0] == '1')
		dnp->dn_flags |= DN_FUNUSABLE;
	if ((ENTRY_VAL(op, F_BOOTP_DN))[0] == '1')
		dnp->dn_flags |= DN_FBOOTP_ONLY;
...
query_to_searchcriteria(const dn_rec_t *dnp, uint_t query, nis_name tablep)
{
	...
	/* flags */
	if (err == DSVC_SUCCESS && (query & DN_QFAUTOMATIC)) {
		i = DSVC_QISEQ(query, DN_QFAUTOMATIC) && (dnp->dn_flags &
		    DN_FAUTOMATIC);
		err = dsvcnis_add_to_qspec(&scp, dn_desc[F_AUTO_DN].tc_name,
		    DSVCNIS_INT, &i);
	}
	if (err == DSVC_SUCCESS && (query & DN_QFMANUAL)) {
		i = DSVC_QISEQ(query, DN_QFMANUAL) && (dnp->dn_flags &
		    DN_FMANUAL);
		err = dsvcnis_add_to_qspec(&scp, dn_desc[F_MANUAL_DN].tc_name,
		    DSVCNIS_INT, &i);
	}
	if (err == DSVC_SUCCESS && (query & DN_QFUNUSABLE)) {
		i = DSVC_QISEQ(query, DN_QFUNUSABLE) && (dnp->dn_flags &
		    DN_FUNUSABLE);
		err = dsvcnis_add_to_qspec(&scp,
		    dn_desc[F_UNUSABLE_DN].tc_name, DSVCNIS_INT, &i);
	}
	if (err == DSVC_SUCCESS && (query & DN_QFBOOTP_ONLY)) {
		i = DSVC_QISEQ(query, DN_QFBOOTP_ONLY) && (dnp->dn_flags &
		    DN_FBOOTP_ONLY);
		err = dsvcnis_add_to_qspec(&scp, dn_desc[F_BOOTP_DN].tc_name,
		    DSVCNIS_INT, &i);
	}
	...
	*listppp = NULL;
	...
	for (i = 0; i <  NIS_RES_NUMOBJ(resp); i++) {
		...
		if (__type_of(nop) == TABLE_OBJ && step(op->zo_name, regp) &&
			...
			if ((ip = calloc(1, INET_ADDRSTRLEN)) == NULL) {
				error = DSVC_NO_MEMORY;
				(*listppp)[*count - 1] = NULL;
				break;
			}
			if (dn_to_ip(op->zo_name, ip, INET_ADDRSTRLEN) != 0) {
				error = DSVC_INTERNAL;
				free(ip);
				(*listppp)[*count - 1] = NULL;
				break;
    ...
    const struct in_addr *netp, const struct in_addr *maskp)
{
	...
	char			netnm[INET_ADDRSTRLEN],
				contnm[sizeof (TMPLT_DN) + 1],
				full_name[NIS_MAXNAMELEN + 1];

	netnbo.s_addr = htonl(netp->s_addr);
	(void) inet_ntop(AF_INET, &netnbo, netnm, sizeof (netnm));
	if (ip_to_dn(netnm, contnm, sizeof (contnm)) != 0)
		return (DSVC_INTERNAL);

	(void) snprintf(full_name, sizeof (full_name), "%s.%s", contnm,
	    location);
	...
	error = dsvcnis_get_tobject(TYPE_DN, contnm, (nis_name)location, &op);
...
remove_dn(const char *location, const struct in_addr *netp)
{
	...
	struct in_addr	netnbo;
	int		error;
	char		netnm[INET_ADDRSTRLEN], contnm[sizeof (TMPLT_DN)],
			full_name[NIS_MAXNAMELEN + 1], sc[NIS_MAXNAMELEN + 1];
	...
	for (;;) {
		error = dsvcnis_get_tobject(TYPE_DN, contnm, (nis_name)location,
		    &op);
		if (error == DSVC_SUCCESS)
			break;
		if (error != DSVC_BUSY)
    ...
    const dn_rec_t *targetp, dn_rec_list_t **recordspp, uint_t *nrecordsp)
{
	...
	for (i = 0, num = 0; i < NIS_RES_NUMOBJ(resp) && (count < 0 ||
		...
		if ((DSVC_QISNEQ(query, DN_QCID) &&
		    ...
		    (targetp->dn_macro != NULL && dnp->dn_macro != NULL &&
...
modify_dn(void *handle, const dn_rec_t *origp, dn_rec_t *newp)
{
	...
	/* flags */
	if ((origp->dn_flags & DN_FAUTOMATIC) !=
	    (newp->dn_flags & DN_FAUTOMATIC))
		ENTRY_FLAGS(nop, F_AUTO_DN) |= EN_MODIFIED;

	if ((origp->dn_flags & DN_FMANUAL) != (newp->dn_flags & DN_FMANUAL))
		ENTRY_FLAGS(nop, F_MANUAL_DN) |= EN_MODIFIED;

	if ((origp->dn_flags & DN_FUNUSABLE) != (newp->dn_flags & DN_FUNUSABLE))
		ENTRY_FLAGS(nop, F_UNUSABLE_DN) |= EN_MODIFIED;

	if ((origp->dn_flags & DN_FBOOTP_ONLY) !=
	    (newp->dn_flags & DN_FBOOTP_ONLY))
		ENTRY_FLAGS(nop, F_BOOTP_DN) |= EN_MODIFIED;
