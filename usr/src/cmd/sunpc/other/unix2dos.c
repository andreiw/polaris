...
main(int argc, char **argv)
{
		...
		if (strcmp(sysinfo_str, "i386")) {
			if ((kbdfd = open("/dev/kbd", O_WRONLY)) < 0) {
				fprintf(stderr,
				    ...
				    "keyboard type US keyboard assumed\n");
			}
    ...
    do {
		num_read = fread(&tmp_buff[100], 1, 100, in_stream);
		i = 0;
		out_len = 0;
		...
		if (out_len && out_len != fwrite(&tmp_buff[0], 1, out_len,
		    out_stream))
	...
	if (same_name) {
		...
		setmode(fileno(out_stream), O_BINARY);
#endif
		while ((num_read = fread(tmp_buff, 1, sizeof (tmp_buff),
		    in_stream)) != 0) {
			if (num_read != fwrite(tmp_buff, 1, num_read,
			    out_stream))
