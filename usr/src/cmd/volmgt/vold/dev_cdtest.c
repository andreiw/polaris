...
#pragma ident	"@(#)dev_cdtest.c	1.3	05/06/08 SMI"
...
static bool_t	cdtest_use(char *, char *);
static bool_t	cdtest_error(struct ve_error *);
static int	cdtest_getfd(dev_t);
static void	cdtest_close(char *, dev_t);
static void	cdtest_devmap(vol_t *, int, int);
static void	cdtest_thread(char *);


static struct devsw cdtestdevsw = {
	cdtest_use,		/* d_use */
	cdtest_error,		/* d_error */
	cdtest_getfd,		/* d_getfd */
	NULL,			/* d_poll */
	cdtest_devmap,		/* d_devmap */
	cdtest_close,		/* d_close */
	NULL, 			/* d_eject */
	NULL, 			/* d_find */
	NULL,			/* d_check */
	CDTEST_MTYPE,		/* d_mtype */
	CDTEST_CLASS,		/* d_dtype */
	(ulong_t)0 		/* d_flags */
};
...
/*ARGSUSED*/
static bool_t
cdtest_use(char *path, char *symname)
{
	...
	if (ioctl(fd, VTIOCUNITS, &nunits) < 0) {
		if (errno == ENOTTY) {
#ifdef	DEBUG
			debug(6, "cdtest_use: found OLD test driver ioctls\n");
#endif
			...
			warning(
			    "cdtest: VTIOCUNITS ioctl failed for \"%s\"; %m\n");
			return (FALSE);
		}
	}
#ifdef	DEBUG
	else {
		debug(6, "cdtest_use: found NEW test driver ioctls\n");
	}
#endif
	...
	for (i = 1; i < nunits; i++) {
		...
		if ((dp = dev_makedp(&cdtestdevsw, namebuf)) == NULL) {
			/* only happens if path spec'ed in namebuf is bogus */
			warning("cdtest: bailing (on \"%s\")\n", namebuf);
			return (FALSE);
		}
		...
		tp->t_fd = -1;
		/*
		 * (void) sprintf(namebuf, "/dev/voltestdrv/%d", i);
		 */
		(void) snprintf(namebuf, sizeof (namebuf),
		    "/dev/voltestdrv/%d", i);
		/* we only have character devices */
		dp->dp_rvn = dev_dirpath(namebuf);
		(void) sprintf(namebuf, "test%d", i);
		dp->dp_symname = strdup(namebuf);
	}
	if (close(fd) < 0) {
		warning("cdtest_use: close of \"%s\" failed; %m\n", namebuf);
	}
	(void) snprintf(namebuf, sizeof (namebuf), "%s/%s", path, VTCTLNAME);
	if (thr_create(0, TEST_STKSIZE, (void *(*)(void *))cdtest_thread,
	    (void *)strdup(namebuf), THR_BOUND, (thread_t *)&ttid) < 0) {
		warning(gettext("cdtest_use: thread create failed; %m\n"));
		return (FALSE);
	}
#ifdef	DEBUG
	debug(6, "cdtest_use: cdtest_thread id %d created\n", ttid);
#endif
	return (TRUE);
}
...
/*ARGSUSED*/
static bool_t
cdtest_error(struct ve_error *vie)
{
	debug(1, "cdtest_error\n");
	return (TRUE);
}
...
/*ARGSUSED*/
static void
cdtest_close(char *path, dev_t rdev)
{
	/* do nothing */
	debug(1, "cdtest_close: called for \"%s\" -- ignoring\n", path);
}


/*ARGSUSED*/
static void
cdtest_devmap(vol_t *v, int part, int off)
{
...
cdtest_thread(char *path)
{
...
#ifdef	DEBUG
	debug(9, "cdtest_thread: entering for \"%s\"\n", path);
#endif
	...
	while (1) {
...
#ifdef	DEBUG
		debug(9, "cdtest_thread: waiting for a VT event ...\n");
#endif
		...
		debug(1, "cdtest_thread: insert on unit %d\n", vte.vte_dev);
		...
		if (tp->t_fd < 0) {
#ifdef	DEBUG
			debug(9, "cdtest_thread: opening \"%s\" RO\n",
			    dp->dp_path);
#endif
			if ((tp->t_fd = open(dp->dp_path, O_RDONLY)) < 0) {
				warning(
			"cdtest_thread: read-only open of \"%s\" failed; %m\n",
				    dp->dp_path);
			} else {
				/* set close-on-exec */
...
#ifdef	DEBUG
		debug(9, "cdtest_thread: creating insert event for (%d.%d)\n",
		    major(dev), minor(dev));
#endif
