...
#pragma ident	"@(#)installgrub.c	1.3	05/08/08 SMI"
...
get_start_sector()
{
	...
	for (i = 0; i < FD_NUMPART; i++) {
		...
		if (is_bootpar) {
			...
			if (part->systid == 0x82 || part->systid == 0xbf)
				break;
		}
	}
...
modify_and_write_stage1(int dev_fd)
{
	if (is_floppy) {
		...
		stage2_first_sector = get_start_sector() + blocklist[0];
		...
		*((unsigned char *)(stage1_buffer + STAGE1_FORCE_LBA)) = 1;
		...
		stage2_first_sector = get_start_sector() + STAGE2_BLKOFF;
...
modify_and_write_stage2(int dev_fd)
{
	...
	if (is_floppy || is_bootpar) {
		...
		if (is_floppy)
			...
			partition_offset = get_start_sector();
