...
#pragma ident	"@(#)error.c	1.3	05/06/08 SMI"
...
err_short(fmd_log_t *lp, const fmd_log_record_t *rp, FILE *fp)
{
	...
	fmdump_printf(fp, "%-15s.%4.4llu %-32s\n",
	    fmdump_date(buf, sizeof (buf), rp),
	    rp->rec_nsec / (NANOSEC / 10000), rp->rec_class);
...
err_verb1(fmd_log_t *lp, const fmd_log_record_t *rp, FILE *fp)
{
	...
	fmdump_printf(fp, "%-15s.%4.4llu %-37s 0x%016llx\n",
	    fmdump_date(buf, sizeof (buf), rp),
	    rp->rec_nsec / (NANOSEC / 10000), rp->rec_class, ena);
