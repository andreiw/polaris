...
#pragma ident	"@(#)piclenvsetup.c	1.6	05/10/11 SMI"
...
add_disk_nodes_and_props()
{
	...
	for (i = 0; i < N_DISK_NODES; i++) {
		if (env_debug)
			envd_log(LOG_ERR, "adding disk nodes...\n");
		dnodep = &disk_nodes[i];
		/*
		 * Get the parent nodeh
		 */
...
add_fan_nodes_and_props()
{
	...
	for (i = 0; i < N_FAN_NODES; i++) {
		...
		err = add_volatile_prop(cnodeh, pname, PICL_PTYPE_INT,
		    ...
		    get_current_speed, set_current_speed, &proph);
...
env_picl_setup_tuneables(void)
{
	 ...
	 */
	if (env_debug)
		envd_log(LOG_ERR, "parsing tuneables...\n");
	env_parse_tunables(rooth);
	for (i = 0; i < ntuneables; i++) {
		if (env_debug)
			envd_log(LOG_ERR, "tuneable %d being added\n", i);
...
env_picl_setup(void)
{
	...
	if (err != PICL_SUCCESS) {
		if (fan_err != PICL_SUCCESS)
			delete_fan_nodes_and_props();
		if (disk_err != PICL_SUCCESS)
			delete_disk_nodes_and_props();
		if (sensor_err != PICL_SUCCESS)
			delete_sensor_nodes_and_props();
	...
	if (env_debug)
		envd_log(LOG_ERR, "parsing the envmodel.conf file...\n");
