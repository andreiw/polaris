...
#pragma ident	"@(#)piclenvsetup.c	1.6	05/06/08 SMI"
...
/*
 * PICL Classes and standard property names
 */
...
#define	UNITADDR_LEN_MAX	256	/* max length for UnitAddress */
...
/*
 * Various temperature threshold property names
 */
...
static sensor_node_t sensor_nodes[] = {
	{SENSOR_CPU_DIE, NULL, CPU_TEMPDEV_UNITADDR,
	    CPU_PLAT_PATH, PROP_CPU_DIE_TEMP},

	{SENSOR_CPU_AMB, NULL, CPU_TEMPDEV_UNITADDR,
	    CPU_PLAT_PATH, PROP_CPU_AMB_TEMP},
...
get_current_temp(ptree_rarg_t *parg, void *buf)
{
	 ...
	 */
	proph = parg->proph;
	for (snodep = &sensor_nodes[0]; snodep->sensor_name != NULL; snodep++) {
