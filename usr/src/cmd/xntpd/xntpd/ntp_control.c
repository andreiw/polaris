...
#pragma ident	"@(#)ntp_control.c	1.3	01/04/12 SMI"
...
static	void	log_buf_overflow	P((struct in_addr,struct in_addr));
 ...
 */
static void
log_buf_overflow(struct in_addr from, struct in_addr to)
{
	struct timeval now;
	...
	if (now.tv_sec - last_log_sec > LOG_INTERVAL_SECS) {
		last_log_sec = now.tv_sec;
		log_count = 0;
	}

	if (log_count < LOG_MSGS_PER_INTERVAL) {
		(void) strlcpy(from_s, inet_ntoa(from), sizeof(from_s));
		log_count++;
		msyslog(LOG_ERR|LOG_AUTH,
