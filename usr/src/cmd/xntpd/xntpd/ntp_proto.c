...
#pragma ident	"@(#)ntp_proto.c	1.4	03/05/30 SMI"
	...
	struct recvbuf *rbufp;
{
	...
	if ((restrict_mask & RES_DONTSERVE) &&
	    (PKT_MODE(pkt->li_vn_mode) == MODE_CLIENT))
		return;
	...
	if (peer == 0) {
		...
		case MODE_ACTIVE:
			...
			if (sys_authenticate && !is_authentic) {
				...
				if (timenow.tv_sec > (timelogged.tv_sec +
				    PEERAUTH_LOG_INTERVAL)) {
					msyslog(LOG_WARNING,
				...
				if (debug > 0) {
					printf("receive: aspiring active"
					    ...
					    ntoa(&rbufp->recv_srcadr),
					    pkt->stratum, distrusted_aspiring);
				}
#endif
				return;
			}
