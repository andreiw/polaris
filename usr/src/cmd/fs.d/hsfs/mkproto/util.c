...
#pragma ident	"@(#)util.c	1.6	05/08/23 SMI"
...
struct tm *t;
	...
	t = gmtime(&tt);
	*dp++ = (uchar_t)t->tm_year;
	*dp++ = (uchar_t)t->tm_mon + 1;
	*dp++ = (uchar_t)t->tm_mday;
	*dp++ = (uchar_t)t->tm_hour;
	*dp++ = (uchar_t)t->tm_min;
	*dp++ = (uchar_t)t->tm_sec;
}
...
int offset;
	...
	/* skip dot and dotdot */
	length = (uint_t)IDE_NAME_LEN(dirp);
	c = IDE_NAME(dirp)[0];

	if ((length == 1 && c == '\0') || (length == 1 && c == '\1'))
		return;
	...
	offset = (uint_t)IDE_NAME_LEN(dirp);
	offset = offset & 0x01? offset : offset+1;
	offset = offset + IDE_FDESIZE;
	printf(" %x\t", IDE_UNIX_MODE(dirp + (uint_t)offset));
	printf("%d\t", IDE_UNIX_UID(dirp + (uint_t)offset));
	printf("%d\t", IDE_UNIX_GID(dirp + (uint_t)offset));
	printf("%d\t", IDE_UNIX_NLINK(dirp + (uint_t)offset));
...
/*VARARGS*/
void
fatal(char *fmt, int arg1, int arg2)
{
