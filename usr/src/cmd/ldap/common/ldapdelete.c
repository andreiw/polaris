...
#pragma ident	"@(#)ldapdelete.c	1.9	03/10/03 SMI"
...
usage( void )
{
    fprintf( stderr, gettext("usage: %s [options] [dn...]\n"), ldaptool_progname );
    fprintf( stderr, gettext("options:\n") );
    ldaptool_common_usage( 0 );
    fprintf( stderr, gettext("    -a\t\tBy-pass confirmation question when deleting a branch\n") );
             ...
             for ( datalist = ((Head *)&HeadNode)->last; datalist; datalist = datalist->left ) {
                if (datalist)  {
                   if ((rc = ldaptool_delete_ext_s( ld, (char *)datalist->data, serverctrls, NULL, "ldap_delete" )) == LDAP_SUCCESS && ldaptool_verbose ) {
                      printf( gettext("%s entry removed\n"), (char *)datalist->data );
                   }
