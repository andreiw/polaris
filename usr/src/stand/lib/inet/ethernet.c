...
/*	char	filler[ETHERMIN - USED_SIZE]; */
};

struct arp_packet arppket_buf;
...
ether_comarp(struct arp_packet *out, uint32_t timeout)
{
	...
	int count, time, feedback, len, delay = 5;
	char    *ind = "-\\|/";
	...
	for (count = 0; timeout == ~0U || prom_gettime() < wait_time; count++) {
		...
		dprintf("RARP package:\n");
		dprintf("\tdest: %s\n", ether_print((uchar_t *)&out->arp_eh.ether_dhost));
		dprintf("\tsrc:  %s\n", ether_print((uchar_t *)&out->arp_eh.ether_shost));
		dprintf("\ttype: %x\n", out->arp_eh.ether_type);

		dprintf("netif_put (by ethernet ihandle %x) to mac %s\n", mac_state.mac_dev, ether_print((uchar_t *)&out->arp_ea.arp_tha));

		/* replaced with the below netif_put */
		/* (void)prom_write(mac_state.mac_dev, (caddr_t)out_buf,
		    sizeof (*out), 0, NETWORK);
		*/

		netif_put(mac_state.mac_dev, (caddr_t)out,
		    sizeof (*out));
		...
		while (prom_gettime() <= time) {
			len = netif_get(mac_state.mac_dev, (caddr_t)in,
			    mac_state.mac_mtu);
...
ether_revarp(void)
{
	...
	printf("Internet address (via rarp) is: %s\n",
		inet_ntoa(ip));

	/*
	ip._S_un._S_un_b.s_b1 = 192;
	ip._S_un._S_un_b.s_b2 = 168;
	ip._S_un._S_un_b.s_b3 = 1;
	ip._S_un._S_un_b.s_b4 = 12;
	*/
...
ether_input(int index)
{
	...
	uint32_t	pre_pr, post_pr; /* netif_get (prom_read) interval */

#ifdef	DEBUG
	int		failures = 0;		/* total failures */
	int		total_attempts = 0;	/* total netif_get (prom_read) */
	int		no_data = 0;		/* no data in prom */
	int		arps = 0;		/* arp requests processed */
	uint32_t	tot_pr = 0;		/* netif_get (prom_read) time */
	...
	do {
		 ...
		 * where netif_get (prom_read()) returns a nonzero length, even when it's
		 ...
		 * the timeout by calling the time spent in netif_get (prom_read()) "free".
		 * netif_get (prom_read()) returns the number of bytes actually read, but
		 * will only copy "len" bytes into our buffer. Adjust in
		 * case the MTU is wrong.
		 */
		pre_pr = prom_gettime();
		len = netif_get(mac_state.mac_dev, mac_state.mac_buf,
		    mac_state.mac_mtu);
		...
		if (eh->ether_type == ntohs(ETHERTYPE_ARP) &&
			...
			if (ea->arp_op == ntohs(ARPOP_REQUEST) &&
				...
				/* replaced by netif_put */
				/* (void) prom_write(mac_state.mac_dev,
				    out_buf, sizeof (struct arp_packet), 0, NETWORK); */
				netif_put(mac_state.mac_dev, mac_state.mac_buf,
...
ether_output(int index, struct inetgram *ogp)
{
	...
	/* replaced by netif_put */
	/* return (prom_write(mac_state.mac_dev, (caddr_t)mp->b_rptr, size,
	    0, NETWORK)); */
	return netif_put(mac_state.mac_dev, (caddr_t)mp->b_rptr, size);
}
