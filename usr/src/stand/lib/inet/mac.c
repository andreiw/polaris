...
#pragma ident	"@(#)mac.c	1.11	06/01/17 SMI"
...
mac_init(char *bootdevicename)
{
	...
	static char 	*nettype = "network-type";
	pnode_t		node;
	char		*wp, *media_type;
	int		len = 0, i;
#endif	/* !__i386 */
	 ...
	 */
	node = prom_finddevice(bootdevicename);
	if (node != OBP_NONODE && node != OBP_BADNODE) {
		...
		} else if((len = prom_getproplen(node, nettype)) >0 &&
		    len < sizeof (tmpbuf)) {
			(void) prom_getprop(node, nettype, tmpbuf);
		}
	...
	default:
		...
		if (mac_state.mac_mtu == 0)
			mac_state.mac_mtu = ETHERSIZE;
#ifdef DEBUG
		printf("ethernet max-frame-size is %x\n", mac_state.mac_mtu);
#endif /* DEBUG */
		mac_state.mac_addr_len = sizeof (ether_addr_t);
		mac_state.mac_addr_buf = bkmem_alloc(mac_state.mac_addr_len);
		if (mac_state.mac_addr_buf == NULL)
		...
		prom_dma_alloc(mac_state.mac_dev, 64*1024, &mac_state.txdma_buf);
		break;
	}
...
mac_fini()
{
	...
	if (mac_state.txdma_buf != NULL) {
		prom_dma_free(mac_state.mac_dev, 64*1024, mac_state.txdma_buf);
		mac_state.txdma_buf = NULL;
	}
...
/*
 * Send a packet via promif ethernet.
 * Return the length sent (or -1 on error).
 */
size_t
netif_put(ihandle_t ifd, caddr_t out_buf, size_t len)
{
	size_t rv;
	if (ifd == (ihandle_t)-1)
		return -1;

	if (mac_state.txdma_buf) {
		bcopy(out_buf, mac_state.txdma_buf, len);
		out_buf = mac_state.txdma_buf;
	}
	rv = prom_write(ifd, out_buf, len, 0, NETWORK);
 ...
 */
size_t
netif_get(ihandle_t ifd, caddr_t in_buf, size_t maxlen)
{
	size_t rv;
	if (ifd == (ihandle_t)-1)
		return -1;

	rv = prom_read(ifd, in_buf, maxlen, 0, NETWORK);
