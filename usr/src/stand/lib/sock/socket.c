...
get_source_port(boolean_t reserved)
{
	...
	/*
	 * Try and randomize the start of the reserved ports to use
	 * This keeps a quick back to back reboot from having to wait
	 * for the server to timeout the old connection.
	 */
	if (rsvdport == 0) {
		rsvdport = prom_gettime() % (IPPORT_RESERVED / 2);
		rsvdport += IPPORT_RESERVED / 2;
	}
	if (reserved) {
