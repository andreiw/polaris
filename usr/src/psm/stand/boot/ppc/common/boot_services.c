...
#pragma ident	"@(#)boot_services.c	1.14	05/06/08 SMI"
...
print_services()
{
	...
	struct boot_nm2svc *pnm2svc = &boot_nm2svc[0];
	prom_printf("Available boot services from %x:\n\
	    \tService Name\tfunc ptr\tversion\n", boot1275_entry);
	while (pnm2svc->b_name != 0) {
		prom_printf("\t%s\t\t\t%x\t\t\t%d\n",
		    pnm2svc->b_name, pnm2svc->b_funcptr,
		    pnm2svc->b_svcversion);
		pnm2svc++;
	}
}
