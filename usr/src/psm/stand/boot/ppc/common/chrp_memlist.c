...
#pragma ident	"@(#)chrp_memlist.c	1.18	05/10/30 SMI"
 ...
 * list.  For ppc, the memory list properties are {hi, lo, size_hi, size_lo},
 * which is similar to boot's format, except boot's format is a linked
 * list, and the prom's is an array of these structures. 
 */
...
static struct memlist *reg_to_list(struct ppc_prom_memlist *a, size_t size,
    struct memlist *old);
static void sort_reglist(struct ppc_prom_memlist *ar, size_t size);
...
static void
sort_reglist(struct ppc_prom_memlist *ar, size_t n)
{
	int i, j, min;
...
reg_to_list(struct ppc_prom_memlist *ar, size_t n, struct memlist *old)
{
